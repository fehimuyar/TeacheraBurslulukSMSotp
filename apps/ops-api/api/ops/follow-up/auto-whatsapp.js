...
const FOLLOW_UP_MODE = ['all', 'result_unseen', 'viewed_no_appointment', 'appointment_no_show'];
...
function normalizeMode(value) {
  ...
  return FOLLOW_UP_MODE.includes(mode) ? mode : 'all';
}

async function appendBotFollowupEvent({ candidateId, attemptId, mode, jobId }) {
  await query(
    `
...
export default async function handler(req, res) {
  await handleRequest(req, res, async () => {
    ...
    const mode = normalizeMode(body?.mode || req.query?.mode);
    ...
    const resultUnseenDelayMinutes = clampInt(
      body?.result_unseen_delay_minutes
        ...
        ?? req.query?.result_unseen_delay_minutes
    ...
    const viewedNoAppointmentDelayMinutes = clampInt(
      body?.viewed_no_appointment_delay_minutes
        ?? body?.viewedNoAppointmentDelayMinutes
        ?? req.query?.viewed_no_appointment_delay_minutes
        ?? process.env.FOLLOWUP_VIEWED_NO_APPOINTMENT_DELAY_MINUTES
        ?? 180,
      0,
      24 * 60,
    );
    const appointmentNoShowDelayMinutes = clampInt(
      body?.appointment_no_show_delay_minutes
        ?? body?.appointmentNoShowDelayMinutes
        ?? req.query?.appointment_no_show_delay_minutes
        ?? process.env.FOLLOWUP_APPOINTMENT_NO_SHOW_DELAY_MINUTES
        ?? 30,
      0,
      24 * 60,
    );
